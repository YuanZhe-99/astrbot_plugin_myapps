...
1. **MyDay API 刷新适配**：新增待办完成、日评分、财务账户/分类查询、近期体重查询工具。
2. **更完整的 MyDay 写入**：待办添加支持备注、计划日期、提醒、子任务和复发；记账支持分类、日期、币种和转账；体重记录支持体脂、三围、备注和日期。

### 改动

1. MyDay 待办、财务摘要、订阅和体重统计输出消费新版 API 的 enriched payload，展示默认币种折算、总资产、BMI、腰臀比和有效三围。
2. HTTP 请求现在使用 WebUI 配置的 `http_timeout`。
3. MyDay API 文档更新到 `0.8.0` 本地接口契约。
